...
namespace SmartPage.Themes.HebammeKleinesWunder
{
        ...
        public void GetMenu(IContent menu, NavigationBuilder builder)
        {
            if (menu.As<TitlePart>().Title == "Main Menu")
            {
                ...
                if (_orchardServices.WorkContext.CurrentUser != null)
                {
                    ...
                    builder.Add(T("Forum"), itemCount.ToString() + ".1", item => item.Url("/forum"));
                    builder.Add(T("Change Password"), itemCount.ToString() + ".2", item => item.Action("ChangePassword", "Account", new { area = "Orchard.Users" }));
                    builder.Add(T("Sign Out"), itemCount.ToString() + ".3", item => item.Action("LogOff", "Account", new { area = "Orchard.Users", ReturnUrl = _orchardServices.WorkContext.HttpContext.Request.RawUrl }));
                    if (_orchardServices.Authorizer.Authorize(Orchard.Security.StandardPermissions.AccessAdminPanel))
                    {
                        builder.Add(T("Dashboard"), itemCount.ToString() + ".4", item => item.Action("Index", "Admin", new { area = "Dashboard" }));
                    }
                }
                else {
