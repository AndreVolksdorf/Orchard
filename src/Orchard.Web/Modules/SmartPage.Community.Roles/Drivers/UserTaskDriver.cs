...
using SmartPage.Community.Roles.Activities;
...
namespace SmartPage.Community.Roles.Drivers {
    ...
    public class UserTaskDriver : ContentPartDriver<ContentPart> {
        ...
        protected override DriverResult Editor(ContentPart part, dynamic shapeHelper) {
            var results = new List<DriverResult> {
                ContentShape("UserTask_ActionButton", () => {
                    ...
                    var actions = awaiting.Where(x => UserIsInGroup(x, user)).SelectMany(ListAction).ToList();
        ...
        private static bool UserIsInGroup(AwaitingActivityRecord x, IUser user) {
            var state = FormParametersHelper.FromJsonString(x.ActivityRecord.State);
            string rolesState = state.Groups ?? "";
            ...
            return UserTaskActivity.UserIsInGroup(user, roles);
        }
