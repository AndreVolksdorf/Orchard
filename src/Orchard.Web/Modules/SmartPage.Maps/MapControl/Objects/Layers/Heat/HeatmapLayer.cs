...
namespace SmartPage.Maps.MapControl.Objects.Layers.Heat
{
    public class HeatmapLayer : Layer, ILocationContainer
    {
        private readonly List<Location.Location> data;

        public HeatmapLayer(Map map)
            : base("heatmap", map)
        {
            this.Gradient = new Collection<Color>();
            this.data = new List<Location.Location>();
        }

        public ReadOnlyCollection<Location.Location> Data
        {
            get
            {
                return new ReadOnlyCollection<Location.Location>(this.data);
            }
        }
        ...
        public void AddPoint(Location.Location point)
        {
            if (point == null)
            {
